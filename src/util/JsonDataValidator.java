...
public class JsonDataValidator {
	...
	private static boolean validateJsonObjectOfBanner(JSONObject jsonObject) {
		...
		if (bannerId == null || bannerId.isEmpty()) {
			System.out.println("[ERROR] BannerId is empty or invalid for this object : " + jsonObject.toString());
			return false;
		}
		String bannerName = (String) jsonObject.get("bannerName");
		if (bannerName == null || bannerName.isEmpty()) {
			System.out.println("[ERROR] BannerName is empty or invalid for this object : " + jsonObject.toString());
			return false;
		}
		String startDateTime = (String) jsonObject.get("startDateTime");
		if (startDateTime == null || startDateTime.isEmpty()) {
			System.out.println("[ERROR] StartDateTime is empty or invalid for bannerId : " + bannerId);
			return false;
		}
		String endDateTime = (String) jsonObject.get("endDateTime");
		if (endDateTime == null || endDateTime.isEmpty()) {
			System.out.println("[ERROR] EndDateTime is empty or invalid for bannerId : " + bannerId);
			return false;
		}

		// check format of startDateTime and endDateTime
		try {
			DateUtils.getDateInTimezone(startDateTime, ZoneId.of("UTC"));
			DateUtils.getDateInTimezone(endDateTime, ZoneId.of("UTC"));
